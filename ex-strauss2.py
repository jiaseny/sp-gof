"""
Apply KDSD and MMD tests to the 2-D Strauss process.
Samples are drawn using strauss-sample.R and saved as text files.
"""
...
if __name__ == "__main__":
    n, kernel_type, l, beta0, gamma0, r0, beta, gamma, r, true_dist, seed, res_dir = sys.argv[1:]
    ...
    # Samples for p
    samples_p = load_samples(
        res_dir + "strauss-samples-n%d-l%.1f-beta%.3f-gamma%.3f-r%.3f-seed%d.txt" %
        (n, l, beta, gamma, r, 10*seed), num_samples=n)

    # Samples for q
    samples_q = load_samples(
        res_dir + "strauss-samples-n%d-l%.1f-beta%.3f-gamma%.3f-r%.3f-seed%d.txt" %
        (n, l, beta, gamma, r, 10*seed+1), num_samples=n)

    assert len(samples_p) == n
    assert len(samples_q) == n
    ...
    elif kernel_type == 'count':
        kernel_fun = count_kernel
        int_method = 'trapz'
        ksd_method = 'indirect'
